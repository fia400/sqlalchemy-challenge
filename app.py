...
def stations():
    ...
    # Create a dictionary append to a list of all_stations.
    all_stations = []
...
def tobs():
    ...
    """Return a json list of TemperatureObservations (tobs) for the previousyear"""
...
def start_stats(start=None):
    ...
    # Create a dictionary from the row data and append to a list of for temperature data.
    temp_stats = []
...
def calc_stats(start=None, end=None):
    ...
    # Query all the stations for given range
    results = session.query(func.min(Measurement.tobs), func.max(Measurement.tobs),func.avg(Measurement.tobs)).\
    filter(Measurement.date >= start).filter(Measurement.date <= end).all()


    # Create a dictionary append to a list temperature data.
    begin_end_stats = []
